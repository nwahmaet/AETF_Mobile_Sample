...
progress.data.uidprefix = "" + ( Date.now ? Date.now() : (new Date().getTime()));

var UID_MAX_VALUE = 999999999999999; /* 15 - 9 */

progress.data._getNextId = function() {
	var uid = ++progress.data.nextid;
	if (uid >= UID_MAX_VALUE) {
		uid = 1;
		progress.data.uidprefix = "" + ( Date.now ? Date.now() : (new Date().getTime()));
	}
	
	return progress.data.uidprefix + "-" + uid;	
};
...
progress.data.JSDO = function JSDO(resNameOrParmObj, serviceName  ) {
	...
	if ((typeof Object.defineProperty) == 'function') {
		...
		*/
		...
		progress.data.Session.MODE_APPEND = 1;
		progress.data.Session.MODE_EMPTY = 2;
		/*		
		progress.data.Session.MODE_MERGE = 3;
		progress.data.Session.MODE_REPLACE = 4;
		*/
	}
...
//			localServiceName = serviceName;
	...
	this._syncTableRef = function(operation, tableRef, batch) {
		...
		case PROGRESS_JSDO_OP_CREATE:
			for (var i = 0; i < tableRef._added.length; i++) {
				...
				this._execGenericOperation(
					PROGRESS_JSDO_OP_CREATE, jsonObject, request, this._createComplete, this._createSuccess, this._createError);
			}
			break;
		case PROGRESS_JSDO_OP_UPDATE:
			for (var id in tableRef._changed) {
				...
				this._execGenericOperation(
					PROGRESS_JSDO_OP_UPDATE, jsonObject, request, this._updateComplete, this._updateSuccess, this._updateError);
			}
			break;
		}
		...
		if (operation == PROGRESS_JSDO_OP_DELETE) {
			for (var i = 0; i < tableRef._deleted.length; i++) {
				...
				this._execGenericOperation(
					PROGRESS_JSDO_OP_DELETE, jsonObject, request, this._deleteComplete, this._deleteSuccess, this._deleteError);
			}
		}
	};
	...
	this._syncDataSet = function() {
		...
		for (var buf in this._buffers) {
			...
			this._syncTableRef(
				PROGRESS_JSDO_OP_DELETE, tableRef, batch);
		}
		...
		for (var buf in this._buffers) {
			var tableRef = this._buffers[buf];
			this._syncTableRef(
				PROGRESS_JSDO_OP_CREATE, tableRef, batch);
		}
		...
		for (var buf in this._buffers) {
			var tableRef = this._buffers[buf];
			this._syncTableRef(
				PROGRESS_JSDO_OP_UPDATE, tableRef, batch);
		}
	...
	this._syncSingleTable = function() {
		...
		for(var i = 0; i < tableRef._added.length; i++) {
  		...
  		for(var i = 0; i < tableRef._deleted.length; i++) {
            ...
            if (addedRecords[id]) {
                var request = {
                    	xhr : undefined,
                    	operation : PROGRESS_JSDO_OP_DELETE,
        				batch : batch,
        				jsrecord : jsrecord,
        				jsdo : this
        			};
		...
		for(var i = 0; i < tableRef._added.length; i++) {
			...
			var request = {
				...
				jsrecord : jsrecord,
				batch : batch,
				operation : PROGRESS_JSDO_OP_CREATE,
				jsdo : this
			};
            batch.operations.push(request);
		...
		for(var id in tableRef._changed) {
			...
			var request = {
				xhr : xhr,
				jsrecord : jsrecord,
				operation : PROGRESS_JSDO_OP_UPDATE,
				batch : batch,
				jsdo : this,
			};
			xhr.request = request;
    ...
    this._getKey = function (record, keyFields) {
    	...
    	for (var i=0; i < keyFields.length; i++) {
    		keyObject[keyFields[i]] = record[keyFields[i]];
    	}
    ...
    this._equalRecord = function (record1, record2, keyFields) {
    	var match = true;
    	for (var i=0; i < keyFields.length; i++) {
    		match = (record1[keyFields[i]] == record2[keyFields[i]]);
...
*/						
    			...
    			if (keyFields.length == 0 || addMode == progress.data.JSDO.MODE_EMPTY) {
    				...
    				if (this._buffers[tableName]._data.length * jsonObject.length >= 10) {
    					...
    					for (var i=0; i < this._buffers[tableName]._data.length; i++) {
    						...
    						var key = this._getKey(record, keyFields);
    						tmpIndex[key] = record;     						
    					} 
    				...
    				for (var i=0; i < jsonObject.length; i++) {
    					...
    					if (tmpIndex) {
    						var key = this._getKey(jsonObject[i], keyFields);
    						record = tmpIndex[key];
    						...
    						for (var j=0; j < this._buffers[tableName]._data.length; j++) {
    							...
    							if (!record) continue;
    							match = (this._equalRecord(jsonObject[i], record, keyFields));
    							if (match) {
    								// Duplicate found
    								break;
    							}
    ...
    this._mergeUpdate = function (jsonObject, xhr) {
    	...
    	if (this.isDataSet() &&  this._dataSetName) {
			var dataset = jsonObject[this._dataSetName];
			if (dataset) {
				for (var buf in this._buffers) {
					var tableRef = this._buffers[buf]; 
					var data = dataset[buf];
					if (data instanceof Array) {
						for (var i = 0; i < data.length; i++) {
	...
	this.onReadyStateChangeGeneric = function () {
		...
		if (xhr.readyState == 4) {
			...
			try {
				...
				else if (request.response && request.response.results) {
					request.response = request.response.results;
				}				
			} catch(e) {
				request.response = undefined;
			}
			request.results = request.response;			
...
progress.ui.UITableRef = function UITableRef(tableRef) {
	...
	this.addItem = function(format) {
		...
		for (field in this._tableRef.record.data) {
			text = text.replace( new RegExp('{' + field + '}', 'g'), this._tableRef.record.data[field]);
		}
	...
	this._getFormFieldValue = function (fieldName) {
		var value = null;

		if (typeof($) == 'function') {
			if (this._detailPage && this._detailPage.name) {
				field = $("#" + this._detailPage.name + " #" + fieldName);
			}
			if (!field || field.length == 0)
				field = $("#" + this._detailPage.name + " [dsid=" + fieldName + "]");
	...
	this._setFormField = function (fieldName, value) {
		var field = null;
		if (typeof($) == 'function') {
			if (this._detailPage && this._detailPage.name) {
				field = $("#" + this._detailPage.name + " #" + fieldName);
    ...
    this.display = function () {
        ...
        var schema = this._tableRef.getSchema();
        for(var i = 0; i < schema.length; i++) {
			this._setFormField(schema[i].name, this._tableRef.record.data[schema[i].name]);
        }
		...
		if (typeof($) == 'function') {
			...
			if (this._listview.autoLink) {
				...
				$("#" + this._listview.name + " li").each (
					...
					$(this).bind('tap',
					function (event,ui) {
						uiTableRef.getListViewRecord(this);
						uiTableRef.display();
					});
				});
			}
				...
				if (element && element.childElementCount > 0) {
					for (var i = 0; i < element.children.length; i++) {
						element.children[i].onclick = function() {
							uihelper.getListViewRecord(this);
							uihelper.display();
						};
					}
				}	  									
	...
	this.getFormRecord = function() {
		var id = this._getFormFieldValue('_id');
		return this._tableRef.findById(id);
	};
...
progress.ui.UIHelper = function UIHelper() {
    ...
    this.display = function () {
		if (this._defaultUITableRef) {
			this._defaultUITableRef.display();
		}
		else
